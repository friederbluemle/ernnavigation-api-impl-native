...
public class MenuItemDemoFragment extends MiniAppNavigationFragment implements OnNavBarItemClickListener {
    ...
    protected ElectrodeNavigationFragmentDelegate createFragmentDelegate() {
        ...
        // 2. Allow native to provide an @IdRes for the menuItem inorder to handle the menu item click inside Activity's or Fragment's onOptionsItemSelected() method.
        delegate.setMenuItemDataProvider(navigationBarButton -> {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.refresh) {
            Toast.makeText(getActivity(), "Refresh handled inside fragment", Toast.LENGTH_SHORT).show();
            return true;
        }
